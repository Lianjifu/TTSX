...
def check_register_params(request):
    ...
    if not(8 <=len(user_pass1) <= 20):
        flag = False
        add_message(request,'user_pass1','密码长度应该在8-20之间')

    # 判断密码是否相等
    if user_pass1 != user_pass2:
        flag = False
        add_message(request,'user_pass2','两次输入的密码不一致')

    # 判断邮箱是否合法
    reg = '^[a-z0-9][\w\.\-]*@[a-z0-9\-]+(\.[a-z]{2,5}){1,2}$'
    if not re.match(reg,user_mail):
        flag = False
        add_message(request,'user_mail','邮箱格式不一致')
...
def check_address_edit_params(request):
    ...
    user_tale = post(request, 'user_tale')
